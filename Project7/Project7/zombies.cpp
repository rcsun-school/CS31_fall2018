...
private:
	...
	int    m_row;
	int    m_col;
	bool isInjured; //member indicating whether the zombie has been attacked before
};
...
bool Zombie::getAttacked(int dir)  // return true if dies
{
	...
	if (!isInjured) {
		if (!m_arena->determineNewPosition(m_row, m_col, dir))  //if zombie is adjacent to wall, zombie dies
			return true;
		
		else {
			isInjured = true; //zombie is not adjacent to the wall, zombie survives
			return false;
		}
	}
	return true;  
...
void Player::moveOrAttack(int dir)
{
	...
	//check for attack
	switch (dir) {
		//attack zombie if adjacent in given direction
	case UP:
...
{
	int countZombies = 0; //traverse the array and count zombies
	for (int i = 0; i < m_nZombies; i++) {
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
	if (numZombiesAt(r, c) > 0)
		for (int target = 0; target < m_nZombies; target++) {
			if (m_zombies[target]->row() == r && m_zombies[target]->col() == c) { //redundant
				if (m_zombies[target]->getAttacked(dir)) {
					delete m_zombies[target];
					for (int i = target + 1; i < m_nZombies; i++) {
						m_zombies[i - 1] = m_zombies[i];
					}
					m_nZombies--;
					return true;
				}
